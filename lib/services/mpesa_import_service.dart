...
class MpesaImportService {
  ...
  static app_models.Transaction _convertToTransaction(MpesaSmsTransaction mpesaTransaction, String userId, {String? accountId}) {
    // Store expenses as negative amounts to be consistent with manual
    // transaction entries and analytics that query for amount < 0.
    final double signedAmount = -mpesaTransaction.amount.abs();
  ...
  static Future<MpesaImportResult?> autoImportIfNeeded({
    ...
    try {
      ...
      if (!hasPermission) {
        developer.log('SMS permission not granted; skipping automatic M-Pesa import', name: _logName);
        return null;
      }

      final stats = await getImportStatistics();
      final lastImportDate = stats['lastImportDate'] as DateTime?;
      if (lastImportDate != null) {
        final diff = DateTime.now().difference(lastImportDate);
        if (diff < minInterval) {
          developer.log('Last M-Pesa import was recent; skipping automatic import', name: _logName);
          return null;
        }
      }
      ...
      developer.log('Error during automatic M-Pesa import: $e', name: _logName);
      return null;
    }
  }
